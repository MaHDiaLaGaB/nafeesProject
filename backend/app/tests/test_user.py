...
def override_user_deps(monkeypatch):
    ...
    monkeypatch.setattr(
        get_user_service, "__wrapped__",
        lambda db=None: StubUserService()
    )
    # stub out CurrentUser → always return our dummy_current
    monkeypatch.setattr(
        CurrentUser, "__call__",
        lambda self: dummy_current
    )
...
def test_update_user(client, supabase_auth_token):
    ...
    headers = {"Authorization": f"Bearer {supabase_auth_token}"}
    resp = client.put(f"/users/update/{TEST_USER_ID}", json=update_payload, headers=headers)
    assert resp.status_code == 200
    data = resp.json()
    assert data["id"] == str(TEST_USER_ID)
    ...
    bad = client.put(f"/users/update/{uuid.uuid4()}", json=update_payload, headers=headers)
    assert bad.status_code == 404
