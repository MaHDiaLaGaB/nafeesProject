...
class UserService:
    ...
    async def update_user(
        self, user_id: UUID, user_data: UserUpdate, current_user: UserOut
    ) -> UserOut:
        ...
        if (
            ...
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Cannot modify superadmin"
            )

        try:
            updated = await self.user_crud.update(
    ...
    async def delete_user(self, user_id: UUID, current_user: UserOut) -> None:
        ...
        if (
            ...
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Cannot delete superadmin"
            )
        try:
    ...
    async def get_user_by_email(self, email: str) -> UserOut:
        ...
        if not user:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"Email {email} not found"
            )
        return user
